...
module.exports.functional.spotcheck = function(test) {
  test('spot checks', function(t) {
    ...
    var count1 = sqlite3.count( paths.db.address, 'address', 'WHERE id=137' );
    t.equal(count1, 25);

    // counts for a specific street (open addresses)
    var count2 = sqlite3.count( paths.db.address, 'address', 'WHERE id=137 AND source="OA"' );
    t.equal(count2, 11);

    // counts for a specific street (vertexes)
    var count3 = sqlite3.count( paths.db.address, 'address', 'WHERE id=137 AND source="VERTEX"' );
    t.equal(count3, 14);
...
module.exports.functional.end_to_end = function(test) {
  test('end to end', function(t) {
    ...
    var rows = sqlite3.exec( paths.db.address, 'SELECT * FROM address WHERE id=137 ORDER BY housenumber' );
    t.deepEqual(rows, [
...
module.exports.functional.geojson = function(test) {
  action.geojson(test, paths, 'id=137');
};

// write tsv to disk
module.exports.functional.tsv = function(test) {
  action.tsv(test, paths, 'id=137');
};
